...
public class CPHInline
{
	...
	private bool Linkblock(bool botmessages)
	{
		...
		string pathToWhiteList = args["WhiteListPath"].ToString();
		List<string> whitelist = new List<string>()
		{
			"twitch.tv/" + streamerName,
		};
		bool whitelisted = false;
		string groupName = args["permitgroup"].ToString();
		bool linkpermitgroup = bool.Parse(args["linkpermitgroup"].ToString());
		string permitUser = CPH.GetGlobalVar<string>("permitUser", false);
		string chatterName = args["user"].ToString();
		if ((!CPH.UserInGroup(chatterName, Platform.Twitch, groupName) || !linkpermitgroup) && (permitUser == null || permitUser == "" || chatterName != permitUser))
		{
			...
			string linkdetect = @"[A-Za-z0-9ÁÀȦÂÄǞǍĂĀÃÅǺǼǢĆĊĈČĎḌḐḒÉÈĖÊËĚĔĒẼE̊ẸǴĠĜǦĞG̃ĢĤḤáàȧâäǟǎăāãåǻǽǣćċĉčďḍḑḓéèėêëěĕēẽe̊ẹǵġĝǧğg̃ģĥḥÍÌİÎÏǏĬĪĨỊĴĶǨĹĻĽĿḼM̂M̄ʼNŃN̂ṄN̈ŇN̄ÑŅṊÓÒȮȰÔÖȪǑŎŌÕȬŐỌǾƠíìiîïǐĭīĩịĵķǩĺļľŀḽm̂m̄ŉńn̂ṅn̈ňn̄ñņṋóòôȯȱöȫǒŏōõȭőọǿơP̄ŔŘŖŚŜṠŠȘṢŤȚṬṰÚÙÛÜǓŬŪŨŰŮỤẂẀŴẄÝỲŶŸȲỸŹŻŽẒǮp̄ŕřŗśŝṡšşṣťțṭṱúùûüǔŭūũűůụẃẁŵẅýỳŷÿȳỹźżžẓǯßœŒçÇ\-]+\.(" + domainList + @")(?: |\/|:|$|\\)";
			...
			foreach (Match m in Regex.Matches(message, linkdetect + ipdetect, regexOptions))
			{
				...
				if (!allowed)
				{
					...
					bool firstMessage = bool.Parse(args["firstMessage"].ToString());
					// Do you want to ban first word offenders?
					bool banfirstlinkoffender = bool.Parse(args["banfirstwordoffender"].ToString());
					if (firstMessage && banfirstlinkoffender)
					{
						CPH.TwitchBanUser(chatterName, "Spambot", false);
					}
